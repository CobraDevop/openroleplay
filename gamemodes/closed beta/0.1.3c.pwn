...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	{		
		if(GarbageJobCount[playerid] <= 10)
		{
			GarbageJobCount[playerid] += 1;
			
			new string[256];
			format(string, sizeof(string), "> %s lowers the arm on the truck and extends it to the rubbish bin, picking it up", GetName(playerid));
			SendNearbyMessage(playerid,30.0, COLOR_PURPLE, string);
		...
		else if(GarbageJobCount[playerid] == 11)
		{
			...
			SetPlayerCheckpoint(playerid, 2188.8066,-1988.3834,13.5469, 5.0);
		}
			    ...
			    switch(listitem)
			    {
					case 0:
					{
						SendClientMessage(playerid, COLOR_YELLOW, "*** Open Roleplay Generic Commands ***");
      					SendClientMessage(playerid, COLOR_YELLOW, "Community:{FFFFFF} /staff /helpme /assistance /report /commands /guide /SendFactionRadioMessage");
						SendClientMessage(playerid, COLOR_YELLOW, "Chat:{FFFFFF} /w(hisper) /s(hout) /o(oc) /g(lobal) /clearchat /radio(r) /pm");
						SendClientMessage(playerid, COLOR_YELLOW, "Phone:{FFFFFF} /phonebook /phonenumber /togglephone /call /pickup /endcall");
						...
						SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /getlicense /licenses /showlicenses /pockets /change /shop /tie /gps /cancelgps");
						SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /rappel /hack /rob /hw(hotwire) /gate /backaccount");
		...
		case DIALOG_PLAYER_STATS:
  		{
  		    ...
  		    else
  		    {
				...
				format(dialogstring, sizeof(dialogstring), " Faction: \t%s\n Faction Rank: \t%s\n Job: \t%s \n Coins: \t%i\n Money: \t$%i\n Bank: \t$%d\n\n Houses: \t%d/2\n Vehicles: \t%d/2\n Businesses: \t%d/2", fstring, frankstring, jstring, PlayerData[playerid][Character_Coins], PlayerData[playerid][Character_Money], PlayerData[playerid][Character_Bank_Money], PlayerData[playerid][Character_Total_Houses], PlayerData[playerid][Character_Total_Vehicles], PlayerData[playerid][Character_Total_Businesses]);
...
CMD:slap(playerid, params[])
{
    if(PlayerData[playerid][Admin_Level] >= 1 || PlayerData[playerid][Moderator_Level] >= 1 || PlayerData[playerid][Helper_Level] >= 1)
	{
	    ...
	    else
	    {
	        ...
	        SetPlayerPos(playerid, x + dx, y + dy, z + 5);
