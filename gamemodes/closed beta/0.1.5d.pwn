...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	{	
		...
		else
		{
			...
			{
				new string[256];
			    format(string, sizeof(string), "> %s removes from tools from their belt and starts to repair the light", GetRPName(playerid));
			    SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
	...
	if(PizzaJobDeliveryPlayer[playerid] == 1)
	{
		...
		else if(PizzaJobDeliveryCount[playerid] == 5)
		{
			...
			new string[256];
			format(string, sizeof(string), "> %s passes the pizza over to the customer and receives their pay", GetRPName(playerid));
			SendNearbyMessage(playerid,30.0, COLOR_PURPLE, string);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
		    ...
		    if(response)
		    {
			    switch(listitem)
			    {
					case 0:
					{
						...
						SendClientMessage(playerid, COLOR_YELLOW, "Misc:{FFFFFF} /buyfaction /sellfaction /rent /unrent /joinfaction /cancelrequest");
					}
			        case 1:
					{
					    ...
					    else if(PlayerData[playerid][Character_Faction] == 1)
					    {
							...
							{						
						        ...
						        SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /search /cancelbackup(cbu) /backup(bu) /mdc /ticket /arrest /placeincar(pic)");
						        SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /cuff /taser /acceptjob /canceljob /acceptcall /drag");
								SendClientMessage(playerid, COLOR_YELLOW, "Leader:{FFFFFF} /hire /fire /setrank /requests /acceptrequest /rejectrequest /issuelicense /setrankname");
							}
					    }
					    else if(PlayerData[playerid][Character_Faction] == 2)
					    {
							...
							else
							{
						        ...
						        SendClientMessage(playerid, COLOR_YELLOW, "Basic:{FFFFFF} /fradio(factionradio) /fchat(factionchat) /fstaff /gr(adio) /megaphone /duty /joinfaction /quitfaction /gate");
						        SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /fireex /acceptjob /canceljob /acceptcall");
								SendClientMessage(playerid, COLOR_YELLOW, "Leader:{FFFFFF} /hire /fire /setrank /requests /acceptrequest /rejectrequest /setrankname");
							}
					    }
					...
					case 5:
					{
					    ...
					    {
					        SendClientMessage(playerid, COLOR_YELLOW, "*** Open Roleplay VIP Commands***");
						    SendClientMessage(playerid, COLOR_YELLOW, "Basic:{FFFFFF} /vipchat /locker /summon");
						    SendClientMessage(playerid, COLOR_YELLOW, "TIP:{FFFFFF} There is a VIP building located at the beach of Los Santos, head there to relax");
					    }
			        ...
			        case 6:
					{
					    ...
					    else if(PlayerData[playerid][Admin_Level] == 1)
					    {
							...
							SendClientMessage(playerid, COLOR_YELLOW, "Helper:{FFFFFF} /helperchat(h) /helpmetoggle /spectate /endspectate /nudge /slap /playertype /freeze /unfreeze /trueid");
							SendClientMessage(playerid, COLOR_YELLOW, "Moderator:{FFFFFF} /moderatorchat(m) /kick /reports /closereport /gotocar /gotodoor /gotobiz /gotohouse /jetpack");
							SendClientMessage(playerid, COLOR_YELLOW, "Admin Level 1:{FFFFFF} /adminchat(a) /gotols /gotopos /ajail /rajail /ban /offlineban /unban /checkstats");
					    }
		        ...
		        switch(listitem)
		        {
		            case 0:
		            {
						...
						else
						{
						    ...
						    ShowPlayerDialog(playerid, DIALOG_SHOP_TYPE_ONE, DIALOG_STYLE_LIST, "24/7 Convenience Store", "Hotdog ($12)\nSoda ($4)\nSpirits ($30)\nLockpick ($800)\nRope ($200)\nMask ($2500)\nRadio ($3500)", "Purchase", "Close");
						}
		            }
		            case 1:
		            {
		                if(PlayerData[playerid][Character_Money] >= 4)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased some soda from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}
                        else
						{
		            ...
		            case 2:
		            {
		                if(PlayerData[playerid][Character_Money] >= 30)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased some alcohol from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}
						else
						{
		            ...
		            case 4:
		            {
		                if(PlayerData[playerid][Character_Money] >= 200)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased some rope from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}
						else
						{
					...
					case 6:
		            {
		                if(PlayerData[playerid][Character_Money] >= 3500)
		                {
			                ...
			                new updatequery[2000];
					        mysql_format(connection, updatequery, sizeof(updatequery), "UPDATE `user_accounts` SET `character_radio` = '%i' WHERE `character_name` = '%e' LIMIT 1", PlayerData[playerid][Character_Radio], GetName(playerid));
				    		mysql_tquery(connection, updatequery);
		...
		{
		    if(!response) return ShowPlayerDialog(playerid, DIALOG_SHOP_TYPE_THREE, DIALOG_STYLE_LIST, "Electronic Store", "Mobile Phone\nSim Card\nHouse Items", "Next", "Close");
		        ...
		        switch(listitem)
		        {
		            case 0:
		            {
		                if(PlayerData[playerid][Character_Money] >= 500)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Nokia Phone from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
		            ...
		            case 1:
		            {
		                if(PlayerData[playerid][Character_Money] >= 2000)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Samsung Phone from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
					...
					case 2:
		            {
		                if(PlayerData[playerid][Character_Money] >= 2500)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased a iPhone from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
		        ...
		        switch(listitem)
		        {
		            case 0:
		            {
		                if(PlayerData[playerid][Character_Money] >= 250)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Telecom Network simcard from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}
						else
						{
		            ...
		            case 1:
		            {
		                if(PlayerData[playerid][Character_Money] >= 450)
		                {
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Valley Network simcard from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}
						else
						{
		                ...
		                if(PlayerData[playerid][Character_Money] >= 2500)
		                {
							...
							else
							{
								...
								else
								{
									...
									new string[256];
									format(string, sizeof(string), "> %s has just purchased a House Lock from the store", GetRPName(playerid));
									SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
		                ...
		                if(PlayerData[playerid][Character_Money] >= 5500)
		                {
							...
							else
							{
								...
								else
								{
									...
									new string[256];
									format(string, sizeof(string), "> %s has just purchased a House Alarm from the store", GetRPName(playerid));
									SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
				...
				{					
					case 0:
					{
						...
						else
						{
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Number 1 from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}						
					}
					case 1:
					{
						...
						else
						{
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Number 2 from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}						
					}
					case 2:
					{
						...
						else
						{
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Number 3 from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}						
					}
					case 3:
					{
						...
						else
						{
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Number 4 from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}						
					}
					case 4:
					{
						...
						else
						{
			                ...
			                format(string, sizeof(string), "> %s has just purchased a Fat Guy from the store", GetRPName(playerid));
							SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
						}						
					}
				}
...
public RegistrationSpawn(playerid)
{
	...
	SpawnPlayer(playerid);
 	...
 	PlayerData[playerid][Character_Money] =7500;
  	...
  	SendClientMessage(playerid, COLOR_YELLOW, "(Please take your time to go through our quick guides, that can be located at the front desk or use /tutorial!)");
...
public LoginSpawn(playerid)
{
    ...
    if(PlayerData[playerid][Character_Registered] == 1)
    {
        ...
        else if(PlayerData[playerid][Character_House_ID] > 0 && PlayerData[playerid][Character_Hour] != SERVER_HOUR && PlayerData[playerid][Admin_Jail] == 0)
		{
			...
			{			
				SendClientMessage(playerid, COLOR_YELLOW, "[Last Logged In:] %s | VIP Expiry Date: %s", PlayerData[playerid][Character_Last_Login], PlayerData[playerid][Character_VIP_Exp]);
			}
			else 
			{
				SendClientMessage(playerid, COLOR_YELLOW, "[Last Logged In:] %s", PlayerData[playerid][Character_Last_Login]);
			}
		...
		else if(PlayerData[playerid][Character_House_ID] > 0 && PlayerData[playerid][Character_Hour] == SERVER_HOUR && PlayerData[playerid][Admin_Jail] == 0)
		{
		    if(PlayerData[playerid][Character_Pos_X] == 0 && PlayerData[playerid][Character_Pos_Y] == 0 && PlayerData[playerid][Character_Pos_Z] == 0)
			{
				...
				SendClientMessage(playerid, COLOR_YELLOW, "Thank you for logging back into the server, you must have bugged out, you have been spawned in Los Santos!");
			...
			{				
				...
				SendClientMessage(playerid, COLOR_YELLOW, "Thank you for logging back into the server, you have been spawned where you last logged out!");
...
stock SendFactionRadioMessage(factionid, color, const str[])
{
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
		if(IsPlayerConnected(i))
		{
			if(PlayerData[i][Character_Faction] == factionid && factionid == 1 || factionid == 2 || factionid == 3)
			{
	    ...
	    if(PlayerData[playerid][Character_Faction] == 8)
		{
	        ...
	        else if(DudefixJobID == 1)
	        {
		        if(IsPlayerInRangeOfPoint(playerid, 30, 1349.8253,-1399.9447,13.3056))
		        {
		            ...
		            new string[128];
		  			format(string, sizeof(string), "> %s turns off the water supply and starts to slowly move away the peices", GetRPName(playerid));
					SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
			...
			else
			{
				...
				if(targetid != INVALID_PLAYER_ID)
				{
		  		    ...
		  		    if(IsPlayerInRangeOfPoint(playerid, 15.0, tx, ty, tz))
		  		    {
						...
						new string[256];
      					format(string, sizeof(string), "> %s has tasered %s", GetRPName(playerid), GetRPName(targetid));
						SendNearbyMessage(playerid, 30.0, COLOR_PURPLE, string);
    ...
    else
    {
        if(PlayerData[playerid][Character_Radio] > 0)
        {
            ...
            new dstring[256];
   			...
   			format(dstring, sizeof(dstring), "([Radio (%d)] %s: %s)", charfreq, GetName(playerid), params);
...
CMD:vipchat(playerid, params[])
{
	...
	if(PlayerData[playerid][Character_VIP] == 1)
	{
		...
		{
		    SendPlayerServerMessage(playerid, " /vipchat [message]");
		}
		else
		{			
			new dstring[156];
			format(dstring, sizeof(dstring), "[VIP Chat] %s: %s", GetName(playerid), amessage);
			SendHelperMessage(COLOR_PINK, dstring);
...
CMD:summon(playerid,params[])
{
	...
	else
	{
		...
		else if(PlayerData[playerid][Character_VIP] != 0)
		{
			...
			for(new i = 1; i <= MAX_VEHICLES; i++)
			{
				if(VehicleData[i][Vehicle_Type] == 3 && !IsVehicleOccupied(i))
				{
					vehicleid = i;
					break;
				}
...
CMD:trueid(playerid, params[])
{
    ...
    if(PlayerData[playerid][Admin_Level] >= 1 || PlayerData[playerid][Moderator_Level] >= 1 || PlayerData[playerid][Helper_Level] >= 1)
	{
	    ...
	    if(sscanf(params, "i", targetid))
	    {
	        SendPlayerServerMessage(playerid, " /trueid [maskid]");
	    }
	    else
	    {
			new maskid = targetid;
...
CMD:endspectate(playerid, params[])
{
    ...
    if(PlayerData[playerid][Admin_Level] >= 1 || PlayerData[playerid][Moderator_Level] >= 1 || PlayerData[playerid][Helper_Level] >= 1)
	{
	    if(IsPlayerSpectating[playerid] == 1)
		{
			...
			SetSpawnInfo(playerid, NO_TEAM, PlayerData[playerid][Character_Skin_1],  SpectateX[playerid], SpectateY[playerid], SpectateZ[playerid], 0);
		   	SetPlayerInterior(playerid, 0);
		    SetPlayerVirtualWorld(playerid, 0);
...
CMD:checkstats(playerid, params[])
{
	...
	if(PlayerData[playerid][Admin_Level] >= 1)
	{
		...
		if(sscanf(params, "i", targetid))
		{
		    SendPlayerServerMessage(playerid, " /checkstats [targetid]");
		}
		else
		{
			if(!IsPlayerConnected(targetid)) return SendPlayerErrorMessage(playerid, " You cannot target a non existant player id!");
			else if(targetid != INVALID_PLAYER_ID)
			{
				...
				format(string2, sizeof(string2), "Character Level: \t%i | Character Exp: \t%i/8 | Staff Level: \t%s | Faction: \t%s | Faction Rank: \t%s ", PlayerData[targetid][Character_Level] , PlayerData[targetid][Character_Level_Exp], staffname, fstring, frankstring);
...
CMD:vsettype(playerid, params[])
{
	...
	if(PlayerData[playerid][Admin_Level] < 5) return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
	{
		...
		{
			SendPlayerServerMessage(playerid, " /vsettype [0 - Reset | 1 - Public | 2 - Rental | 3 - Driving School | 4 - VIP | 5 - Admin]");
		}
		else
		{	
		    if(vehicleid == 0) return SendPlayerErrorMessage(playerid, " You are not sitting inside a vehicle!");
...
CMD:setvip(playerid, params[])
{
    ...
    if(PlayerData[playerid][Admin_Level] >= 6)
	{
	    ...
	    {
	        SendPlayerServerMessage(playerid, " /setvip [targetid] [vip level] [days]");
		    SendPlayerServerMessage(playerid, " Levels: [1]");
	    }
	    else
	    {
			if(!IsPlayerConnected(targetid)) return SendPlayerErrorMessage(playerid, " You cannot target a non existant player id!");
...
CMD:ddsettype(playerid, params[])
{
    ...
    if(PlayerData[playerid][Admin_Level] < 6) return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
	{
		...
		{
			SendPlayerServerMessage(playerid, " /ddsettype [doorid] [0 = public | 1 > 20 faction id | 9999 - VIP]");
		}
		else
		{
