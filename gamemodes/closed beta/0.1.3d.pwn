...
#define		DIALOG_DRUG_SELECTION		1000
...
public OnPlayerConnect(playerid)
{
	...
	ApplyAnimation(playerid, "BOMBER", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "INT_SHOP", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "ped", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "CHAINSAW", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "HEIST9", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "CRACK", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "SHOP", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
	ApplyAnimation(playerid, "MISC", "NULL", 4.0, false, false, false, false, 0, SYNC_ALL);
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	{	
		...
		else if(GarbageJobCount[playerid] == 12)
		{
			...
			new dstring[256];
			format(dstring, sizeof(dstring), "- You have just emptied your vehicle, feel free to keep going or use /stopjob to complete the task!");
			SendClientMessage(playerid, COLOR_YELLOW, dstring);
			    ...
			    switch(listitem)
			    {
					case 0:
					{
						SendClientMessage(playerid, COLOR_YELLOW, "*** Open Roleplay Generic Commands ***");
      					SendClientMessage(playerid, COLOR_YELLOW, "Community:{FFFFFF} /staff /helpme /assistance /report /commands /guide /jailtime");
						...
						SendClientMessage(playerid, COLOR_YELLOW, "General:{FFFFFF} /rappel /hack /rob /hw(hotwire) /gate /backaccount /smuggle /bake /usedrugs");
						SendClientMessage(playerid, COLOR_YELLOW, "Vehicle:{FFFFFF} /engine /lights /bonnet /boot /rentcar /unrentcar /fillup /recyclecar /changeownership /park");
			        ...
			        case 5:
					{
					    ...
					    else if(PlayerData[playerid][Admin_Level] == 1)
					    {
							...
							SendClientMessage(playerid, COLOR_YELLOW, "Helper:{FFFFFF} /helperchat(h) /helpmetoggle /spectate /endspectate /nudge /slap");
							SendClientMessage(playerid, COLOR_YELLOW, "Moderator:{FFFFFF} /moderatorchat(m) /kick /reports /closereport /gotocar /gotodoor /gotobiz /gotohouse /jetpack");
							SendClientMessage(playerid, COLOR_YELLOW, "Admin Level 1:{FFFFFF} /adminchat(a) /gotols /gotopos /ajail /rajail");
					    }
		    ...
		    if(response)
		    {
		        switch(listitem)
		        {
		            case 0:
		            {
		                if(PlayerData[playerid][Character_Money] >= 2500)
		                {
			                // Check if the person has a house they own first
			
							// Apply the lock to the house ID and update the database	
							
							// Take the persons money and proceed to advise them to use /lock when at their house
						}
						else
						{
						    SendPlayerErrorMessage(playerid, " You do not have enough money to make this purchase!");
		            ...
		            case 1:
		            {
		                if(PlayerData[playerid][Character_Money] >= 5500)
		                {
							...
							// Apply the alarm to the house ID and update the database	
							
							// Take the persons money and proceed to advise them to use /alarm when at their house
						}
						else
						{
						    SendPlayerErrorMessage(playerid, " You do not have enough money to make this purchase!");
		...
		case DIALOG_DRUG_SELECTION:
		{
			...
			else
	        {
                switch(listitem)
			    {
			        case 0:
					{
					    if(PlayerData[playerid][Character_Has_Drugs] == 0) return SendPlayerErrorMessage(playerid, " You do not have enough money to completed this course!");
						else
						{
							...
							ApplyAnimation(playerid, "MISC", "GRAB_L", 4.0, false, false, false, false, 1, SYNC_ALL);
							
							new dstring[256];
							format(dstring, sizeof(dstring), "- You can now use the drugs by typing /usedrugs!");
							SendClientMessage(playerid, COLOR_YELLOW, dstring);
						}
					}
					case 1:
					{
						if(PlayerData[playerid][Character_Has_Drugs] <= 1) return SendPlayerErrorMessage(playerid, " You do not have enough money to completed this course!");
						else
						{
...
public ONE_SECOND_TIMER()
{
		        ...
		        for(new q = 1; q < MAX_BUSINESSES; q++)
		        {
                    if(BusinessData[q][Business_Outside_X] != 0 && BusinessData[q][Business_Sold] == 0)
                    {
			            if(IsPlayerInRangeOfPoint(i, 3.0, BusinessData[q][Business_Outside_X], BusinessData[q][Business_Outside_Y], BusinessData[q][Business_Outside_Z]))
			            {
			                ...
			                {
				            	new tdstring2[500];
							    format(tdstring2, sizeof(tdstring2), "%s~n~~y~Cost: ~w~$%i ~p~(%i Coins)~w~~n~~n~(Tap ENTER_KEY to go inside!)~n~~n~~y~/4~w~ to buy this business!", BusinessData[q][Business_Name], BusinessData[q][Business_Price_Money], BusinessData[q][Business_Price_Coins]);
								PlayerTextDrawSetString(i, PlayerText:Notification_Textdraw, tdstring2);
								PlayerTextDrawShow(i, PlayerText:Notification_Textdraw);
	...
	if(SERVER_MESSAGE_COUNT == 0)
	{
		...
		SendClientMessageToAll(COLOR_GREY, RandomMessages[index]);	
	}
...
CMD:time(playerid, params[])
{
	if(IsPlayerLogged[playerid] == 1)
	{
	    if(PlayerData[playerid][Character_Registered] == 1)
	    {
			...
			else if(PlayerData[playerid][Admin_Jail_Time] == 0 || PlayerData[playerid][Character_Jail_Time] == 0 || IsPlayerInHospital[playerid] == 0)
			{
			    format(dstring, sizeof(dstring), "[TIMER]:{FFFFFF} You do not have anytime to serve!");
				SendClientMessage(playerid, COLOR_ORANGE, dstring);
			}
	    }
	    else return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
    ...
    else
    {
		...
		else
		{
			if(drugtype == 1 && PlayerData[playerid][Character_Drug_Cocaine] > 0)
			{
				...
				ApplyAnimation(playerid, "SHOP", "Smoke_RYD", 4.0, false, false, false, false, 1, SYNC_ALL);
			...
			else if(drugtype == 2 && PlayerData[playerid][Character_Drug_Heroin] > 0)
			{
				...
				new updatequery[2000];
				mysql_format(connection, updatequery, sizeof(updatequery), "UPDATE `user_accounts` SET `character_drug_cocaine` = '%i' WHERE `character_name` = '%e' LIMIT 1", PlayerData[playerid][Character_Drug_Heroin], GetName(playerid));
				mysql_tquery(connection, updatequery);
...
CMD:bake(playerid, params[])
{
    ...
    else
    {
		...
		{
			if(PlayerData[playerid][Character_Has_Drugs] > 0)
			{
				ShowPlayerDialog(playerid, DIALOG_DRUG_SELECTION, DIALOG_STYLE_LIST, "Shack House Kitchen", "Cocaine\nHeorin", "Bake", "Close");
			}
			else
			{
				SendPlayerErrorMessage(playerid, " You do not have any drugs materials on you at this time, go find a mystery dealer!");
			}
		}
	...
	if(PlayerData[playerid][Character_Registered] != 1) return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
	{
		...
		if(sscanf(params, "i", response))
		{
		...
		else
  		{
			...
			{
			    if(PlayerData[playerid][Character_Coins] < FactionData[ficonid][Faction_Price_Money]) return SendPlayerErrorMessage(playerid, " You do not have enough coins to purchase this faction!");
...
CMD:nudge(playerid, params[])
{
    if(PlayerData[playerid][Admin_Level] >= 1 || PlayerData[playerid][Moderator_Level] >= 1 || PlayerData[playerid][Helper_Level] >= 1)
	{
	    ...
	    else
	    {
			...
			new dstring[256];
			format(dstring, sizeof(dstring), "> Admin has nudged you!");
            SendClientMessage(targetid, COLOR_YELLOW, dstring);

            format(dstring, sizeof(dstring), "> You have just nudged %s!", GetName(targetid));
            SendClientMessage(playerid, COLOR_YELLOW, dstring);
	    }
	}
	else return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
...
CMD:slap(playerid, params[])
{
    if(PlayerData[playerid][Admin_Level] >= 1 || PlayerData[playerid][Moderator_Level] >= 1 || PlayerData[playerid][Helper_Level] >= 1)
	{
	    ...
	    else
	    {
	        ...
	        SetPlayerPos(targetid, x + dx, y + dy, z + 5);
