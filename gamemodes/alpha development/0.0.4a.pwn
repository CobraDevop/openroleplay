...
{
    {1439.3849,-2293.1267,13.5469}, // /BANKACCOUNT - LS BANK
	{1439.5317,-2281.4648,13.5469} // ?BANKACCOUNT - LS COUNTY
};
    ...
    switch(dialogid)
    {
		    ...
		    switch(listitem)
		    {
		        ...
		        case 1:
				{
				    ...
				    else if(PlayerData[playerid][Character_Faction] == 1)
				    {
				        if(PlayerData[playerid][Character_Faction_Rank] != 6)
				        {
							...
							format(dialogstring3, sizeof(dialogstring3), "\nBasic: /acceptcall | /duty | /baton | /cuff | /jail | /ticket | /drag");
		        ...
		        case 5:
				{
				    ...
				    else if(PlayerData[playerid][Admin_Level] == 2)
				    {
				        format(titlestring, sizeof(titlestring), "Admin Commands");
						format(dialogstring1, sizeof(dialogstring1), "Level 1: /adminchat(/achat) | /gotopos | /gotols | /adminjail(/ajail) | /removeadminjail(/rajail)");
						format(dialogstring2, sizeof(dialogstring2), "\n\nLevel 2: /mute | /unmute | /kick | /sethealth");
				    ...
				    else if(PlayerData[playerid][Admin_Level] == 4)
				    {
						...
						format(dialogstring2, sizeof(dialogstring2), "\n\nLevel 2: /mute | /unmute | /kick | /sethealth");
						format(dialogstring3, sizeof(dialogstring3), "\n\nLevel 3: /setskin | /setarmor");
						format(dialogstring4, sizeof(dialogstring4), "\n\nLevel 4: /factionban | /rfactionban | /givemoney");
	    ...
	    case DIALOG_BANK_MENU:
	    {
	        switch(listitem)
	        {
	            ...
	            {
                    new bodytext[256];
	                format(bodytext, sizeof(bodytext), "Balance: $0.00\n1.\tWithdraw\n2.\tDeposit");

					ShowPlayerDialog(playerid, DIALOG_BANK_BUSINESS, DIALOG_STYLE_LIST, "Bank Account - Business", bodytext, "Select", "Go Back");
	            }
	            case 2:
	            {
                    new bodytext[256];
	                format(bodytext, sizeof(bodytext), "Market:\tDown 5%\nEconomy:\tUp 12%\nInterest Rates:\tUp 15.3%");

					ShowPlayerDialog(playerid, DIALOG_BANK_STOCK, DIALOG_STYLE_LIST, "Bank Account - Stock", bodytext, "Select", "Go Back");
	            }
	            case 3:
	            {
                    new bodytext[256];
	    ...
	    case DIALOG_BANK_ADEPOSIT:
	    {
	        ...
	        else
	        {
	            if(strval(inputtext) > PlayerData[playerid][Character_Bank_Money])
		        {
		            ShowPlayerDialog(playerid, DIALOG_BANK_AWITHDRAW, DIALOG_STYLE_INPUT, "Bank Account - Deposit", "(ERROR - You cannot deposit more than what you have in your wallet!)\n\nPlease enter in the amount that you would like to deposit:", "Deposit", "Go Back");
		        }
		        else
		        {
		            PlayerData[playerid][Character_Money] -= strval(inputtext);
...
public SaveUserInformation(playerid)
{
	...
	new cquery[2000];
    mysql_format(connection, cquery, sizeof(cquery), "UPDATE `user_accounts` SET `character_last_login` ='%i', `character_minutes` ='%i', `character_pos_x` = '%f', `character_pos_y` = '%f', `character_pos_z` = '%f', `character_pos_angle` = '%f', `character_interior_id` = '%i', `character_virtual_world` = '%i', `character_health` = '%f', `character_armor` = '%f', `character_money` = '%i', `character_coins` = '%i',`character_vip` = '%i', `character_level` = '%i',`character_level_exp` = '%i', `admin_jail` = '%i',`admin_jail_time` = '%i', `admin_jail_reason` = '%s' WHERE `character_name` = '%e' LIMIT 1"
...
// /giveweapon /removeweapon /weaponban
