...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(TruckJobParcelCount[playerid] < 5)
	{
	...
	if(HasPlayerRobbedAmmunation[playerid] > 0)
	{
	    if(HasPlayerRobbedAmmunationPoint[playerid] == 1)
	    {
	        ...
	        GivePlayerWeapon(playerid, 22, 6);
...
public OnAccountCheck(playerid)
{
    if(cache_num_rows() > 0)
    {
	    ...
	    PlayerData[playerid][Character_Money] = cache_get_field_content_int(0, "character_money");
       	...
       	PlayerData[playerid][Character_Bank_Loan] = cache_get_field_content_int(0, "character_bank_loan");
...
public OnLoanCheckCustomer(playerid)
{
    if (cache_num_rows() > 0)
    {
        ...
        {
            loanID = cache_get_field_content_int(i, "loan_id");

            loanAmount = cache_get_field_content_int(i, "loan_amount");
	        cache_get_field_content(i, "loan_name", loanName, connection, 50);
	        cache_get_field_content(i, "loan_reason", loanReason, connection, 50);
...
public OnLoanCheck(playerid)
{
    if (cache_num_rows() > 0)
    {
        ...
        for (new i = 0; i < cache_num_rows(); i++)
        {
            new loaded = cache_get_field_content_int(i, "loan_id");

            LoanData[loaded][Loan_ID] = loaded;
            cache_get_field_content(i, "loan_name", LoanData[loaded][Loan_Name], connection, 50);
            LoanData[loaded][Loan_Amount] = cache_get_field_content_int(i, "loan_amount");
            cache_get_field_content(i, "loan_reason", LoanData[loaded][Loan_Reason], connection, 50);
            LoanData[loaded][Loan_Status] = cache_get_field_content_int(i, "loan_status");
...
public OnLoanDetails(playerid)
{
    if (cache_num_rows() > 0)
    {
        ...
        loanAmount = cache_get_field_content_int(0, "loan_amount");
        loanStatus = cache_get_field_content_int(0, "loan_status");
        cache_get_field_content(0, "loan_name", loanName, connection, 50);
        cache_get_field_content(0, "loan_reason", loanReason, connection, 50);
        
        cache_get_field_content(0, "loan_name", SQL_LOAN_NAME[playerid][0], connection, 50);
...
public OnReportCheck(playerid)
{
    ...
    if (cache_num_rows() > 0)
    {
        for (new i = 0; i < cache_num_rows(); i++)
        {
			...
			format(stringtitle, sizeof(stringtitle), "> {F2F746}Report ID:{FFFFFF} %d - {F2F746}Reported Name:{FFFFFF} %s", reportID, reportTarget);
...
public LoadFaction_Results()
{
	...
	if(rows)
	{
        ...
        FactionData[loaded][Faction_Money] = cache_get_field_content_int(0, "faction_money");
...
public GetNextDealershipFreeVehicleID(playerid)
{
    ...
    if (rows > 0)
    {
        nextID = cache_get_field_content_int(0, "vehicle_id");
