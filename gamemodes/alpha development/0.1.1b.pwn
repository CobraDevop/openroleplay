...
public OnGameModeInit()
{
	...
	CreateDynamicMapIcon(1474.6604, -2286.6621, 42.4205, 5, -1, -1, -1, -1, -1); // VEHICLE RENTAL ICON
			    ...
			    switch(listitem)
			    {
					case 0:
					{
						SendClientMessage(playerid, COLOR_YELLOW, "*** Basic Commands ***");
      					SendClientMessage(playerid, COLOR_WHITE, "Chat: /me | /do | /whipser(/w) | /shout(/s) | /ooc(/o) | /global(/g)");
						...
						SendClientMessage(playerid, COLOR_WHITE, "Vehicle: /park | /engine | /lights | /boot | /bonnet | /lock");
		...
		case DIALOG_PLAYER_STATS:
  		{
  		    ...
  		    else
  		    {
				...
				format(dialogstring, sizeof(dialogstring), " Faction: \t%s\n Faction Rank: \t%s\n Job: \tNone \n Coins: \t%i\n Money: \t$%i\n Bank: \t$%d\n\n Houses: \t%d/2\n Vehicles: \t0/3\n Businesses: \t%d/2", fstring, frankstring, PlayerData[playerid][Character_Coins], PlayerData[playerid][Character_Money], PlayerData[playerid][Character_Bank_Money], PlayerData[playerid][Character_Total_Houses], PlayerData[playerid][Character_Total_Businesses]);
...
public Save_Character_Info_Query_2(playerid)
{
    ...
    mysql_format(connection, playerquery, sizeof(playerquery), "UPDATE `user_accounts` SET \
					...
					, PlayerData[playerid][Character_Has_Drugs], PlayerData[playerid][Character_Has_Food], PlayerData[playerid][Character_Has_Drinks], PlayerData[playerid][Character_Has_Alcohol], PlayerData[playerid][Character_Has_Phone], PlayerData[playerid][Character_Phonenumber], PlayerData[playerid][Character_Has_SimCard], PlayerData[playerid][Character_Name]);
...
CMD:bsettype(playerid, params[])
{
	if(PlayerData[playerid][Admin_Level] != 6) return SendPlayerErrorMessage(playerid, " You do not have access to this feature!");
	{
		...
		if(sscanf(params,"ii", businessid, biztype))
		{
		    ...
		    SendPlayerServerMessage(playerid, " Options: [Donuts (8) | Clothing 2 (9) |]");
		}
		else
		{
		    if(biztype < 1 || biztype > 9) return SendPlayerErrorMessage(playerid, " You need to select a valid business type ID that can be used in-game!");
		    else
		    {
